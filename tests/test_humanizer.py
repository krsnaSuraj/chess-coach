...
from chess_coach.humanizer import (
    Humanizer, PvLine, ComplexityDetector, SessionMetrics,
    _accuracy_for_elo, _top1_rate_for_elo, _top3_cumulative_for_elo,
    _expected_score,
)
...
class TestHumanizerV3:
    def test_disabled_returns_first_move(self):
        ...
        candidates = [
            ...
            {"pv": [chess.Move.from_uci("d2d4")], "score": chess.engine.PovScore(chess.engine.Cp(15), chess.WHITE)},
        ]
    ...
    def test_select_move_returns_valid_move(self):
        ...
        candidates = [
            {"pv": [chess.Move.from_uci("e2e4")], "score": chess.engine.PovScore(chess.engine.Cp(20), chess.WHITE)},
            {"pv": [chess.Move.from_uci("d2d4")], "score": chess.engine.PovScore(chess.engine.Cp(15), chess.WHITE)},
            {"pv": [chess.Move.from_uci("g1f3")], "score": chess.engine.PovScore(chess.engine.Cp(10), chess.WHITE)},
        ]
        move = h.select_move(candidates, board)
        assert move is not None
    ...
    def test_1500_accuracy_calibrated(self):
        ...
        candidates = [
            ...
            {"pv": [chess.Move.from_uci("d2d4")], "score": chess.engine.PovScore(chess.engine.Cp(15), chess.WHITE)},
            {"pv": [chess.Move.from_uci("g1f3")], "score": chess.engine.PovScore(chess.engine.Cp(10), chess.WHITE)},
            {"pv": [chess.Move.from_uci("b1c3")], "score": chess.engine.PovScore(chess.engine.Cp(5), chess.WHITE)},
            {"pv": [chess.Move.from_uci("c2c4")], "score": chess.engine.PovScore(chess.engine.Cp(3), chess.WHITE)},
        ]
