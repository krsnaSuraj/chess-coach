...
class Humanizer:
    ...
    def select_move(
        ...
        if eval_score > 2.5:
            ...
            self._effective_elo = max(float(self.target_elo) - 100, self._effective_elo - drop)
        ...
        error_type = self._roll_error(effective_inaccuracy, effective_mistake, effective_blunder)
        if error_type:
            return self._inject_error(candidates, board, error_type)

        return self._accuracy_weighted_select(candidates, board)

    def _build_ranked(self, multi_pv: list[InfoDict]) -> list[PvLine]:
        ranked: list[PvLine] = []
    ...
    def _human_mistake(self, candidates: list[PvLine], board: chess.Board) -> chess.Move | None:
        if len(candidates) >= 3:
...
class ComplexityDetector:
    ...
    def is_complex(board: chess.Board) -> bool:
        ...
        if board.fullmove_number <= 8 and pieces >= 28:
            return False
        attack_count = sum(
            len(board.attackers(chess.WHITE, sq)) + len(board.attackers(chess.BLACK, sq))
